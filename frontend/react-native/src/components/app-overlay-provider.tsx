...
export function AppOverlayProvider({ children }: PropsWithChildren) {
  ...
  const [notification, setNotification] = useState<AppNotificationRequest>();
  ...
  const notificationMotion = useRef(new Animated.Value(0)).current;
  ...
  useEffect(() => {
    ...
    notificationMotion.setValue(0);
    Animated.timing(notificationMotion, { toValue: 1, duration: 280, easing: Easing.bezier(0.22, 1, 0.36, 1), useNativeDriver: true }).start();
    const timer = setTimeout(() => dismissNotification(), NOTIFICATION_VISIBLE_MS);
    return () => clearTimeout(timer);
    // A new notification restarts its own entrance and dismissal timer.
  ...
  const dismissNotification = (onDismissed?: () => void) => {
    Animated.timing(notificationMotion, { toValue: 0, duration: 160, easing: Easing.out(Easing.cubic), useNativeDriver: true }).start(() => {
  ...
  const notificationStyle = {
    opacity: notificationMotion,
    transform: [{ translateY: notificationMotion.interpolate({ inputRange: [0, 1], outputRange: [-18, 0] }) }, { scale: notificationMotion.interpolate({ inputRange: [0, 1], outputRange: [0.98, 1] }) }],
  };
  const visual = notificationVisual(notification?.type);

  return <>
    {children}
    {notification ? <View pointerEvents="box-none" style={[styles.notificationLayer, { top: Math.max(12, insets.top + 8) }]}>
      <Animated.View style={[styles.notificationAnimated, notificationStyle]}>
        <Pressable accessibilityRole="button" accessibilityLabel={`${notification.title}. ${notification.message}. 자세히 보기`} onPress={() => dismissNotification(notification.onPress)} style={({ pressed }) => [styles.notificationCard, pressed && styles.notificationPressed]}>
          ...
          <View style={styles.notificationActions}>
            <Pressable accessibilityLabel="알림 닫기" hitSlop={8} onPress={(event) => { event.stopPropagation(); dismissNotification(); }} style={styles.notificationClose}><Ionicons name="close" size={17} color={colors.ink500}/></Pressable>
...
const styles = StyleSheet.create({
  ...
  notificationLayer: { position: 'absolute', left: 12, right: 12, zIndex: 1000, alignItems: 'center' },
  ...
  notificationClose: { width: 36, height: 36, alignItems: 'center', justifyContent: 'center', borderRadius: 18, backgroundColor: colors.canvas },
