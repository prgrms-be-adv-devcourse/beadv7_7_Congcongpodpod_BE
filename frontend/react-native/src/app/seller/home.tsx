...
export default function SellerHome() {
  ...
  return <SellerShell title="대시보드" description="놓치면 안 되는 주문과 오늘 매출을 먼저 보여드려요." storeName={store.storeName} storeStatus={store.status} refreshing={refreshing} onRefresh={onRefresh}>
    <View style={styles.operation}><View style={styles.operationCopy}><View style={styles.operationTitleRow}><View style={[styles.operationDot, store.status !== 'OPEN' && styles.operationDotClosed]}/><Text style={styles.operationTitle}>{store.status === 'OPEN' ? '운영 중' : store.status === 'STOPPED' ? '운영 중지' : '오픈 전'}</Text></View><Text style={styles.operationDescription}>{store.status === 'OPEN' ? '고객이 매장과 판매 상품을 확인할 수 있어요.' : store.status === 'STOPPED' ? '관리자에 의해 운영이 중지된 매장이에요.' : '준비가 끝나면 매장을 열어주세요.'}</Text></View><Pressable accessibilityRole="button" disabled={statusChanging || store.status === 'STOPPED'} onPress={() => setPendingStatus(store.status === 'OPEN' ? 'CLOSED' : 'OPEN')} style={({ pressed }) => [styles.statusAction, store.status === 'OPEN' && styles.statusActionClose, (pressed || statusChanging || store.status === 'STOPPED') && styles.statusActionDisabled]}><Text style={[styles.statusActionText, store.status === 'OPEN' && styles.statusActionCloseText]}>{store.status === 'OPEN' ? '매장 닫기' : store.status === 'STOPPED' ? '변경 불가' : '매장 열기'}</Text></Pressable></View>
    ...
    <View style={styles.sectionHead}><View><Text style={styles.heading}>운영 메뉴</Text><Text style={styles.sectionDescription}>자주 쓰는 업무로 바로 이동하세요.</Text></View><Pressable accessibilityRole="button" hitSlop={10} onPress={() => router.push('/seller/store')}><Text style={styles.link}>매장 정보</Text></Pressable></View>
...
const styles = StyleSheet.create({
  operation: { minHeight: 86, padding: 14, flexDirection: 'row', alignItems: 'center', gap: 12, borderRadius: 12, backgroundColor: colors.white, borderWidth: 1, borderColor: colors.line, ...shadow.card },
  ...
  statusAction: { minWidth: 86, minHeight: 44, paddingHorizontal: 13, alignItems: 'center', justifyContent: 'center', borderRadius: 9, backgroundColor: colors.green500 },
  ...
  priority: { padding: 17, borderRadius: 13, backgroundColor: colors.ink900, ...shadow.card },
  ...
  priorityAction: { minHeight: 46, marginTop: 17, paddingHorizontal: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderRadius: 9, backgroundColor: colors.white },
  priorityActionText: { color: colors.ink900, fontFamily: fonts.body, fontSize: 12, fontWeight: '900' },
  metrics: { flexDirection: 'row', gap: 10 },
  metric: { flex: 1, minHeight: 112, padding: 13, borderRadius: 12, backgroundColor: colors.white, borderWidth: 1, borderColor: colors.line, ...shadow.card },
