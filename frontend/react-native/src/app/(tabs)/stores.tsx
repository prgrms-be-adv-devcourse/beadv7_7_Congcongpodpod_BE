...
import { FlatList, Image, Modal, Pressable, ScrollView, StyleSheet, Text, TextInput, View, type GestureResponderEvent } from 'react-native';
...
export default function StoresScreen() {
  ...
  const filteredStores = useMemo(() => {
    ...
    return stores.filter((store) => {
      ...
      const keywordMatch = !keyword || [store.storeName, getStoreCategoryVisual(store.category).label, store.address]
        .some((value) => value.toLocaleLowerCase('ko').includes(keyword));
      return categoryMatch && keywordMatch;
    });
  }, [category, query, stores]);
  const featured = filteredStores.slice(0, 5);
  ...
  const topBar = <View style={[styles.topBar, { width: contentWidth, paddingHorizontal: gutter }]}> 
      <View style={styles.search}><BrandLogo size={30}/><TextInput accessibilityLabel="주변 상점 검색" value={query} onChangeText={setQuery} placeholder="내 주변 매장·상품 검색" placeholderTextColor={colors.ink500} returnKeyType="search" style={styles.searchInput}/>{query ? <Pressable accessibilityLabel="검색어 지우기" hitSlop={10} onPress={() => setQuery('')}><Ionicons name="close-circle" size={18} color={colors.ink400}/></Pressable> : null}</View>
  ...
  const header = <View style={[styles.header, { width: contentWidth }]}> 
    ...
    <View style={[styles.radiusSection, { marginHorizontal: gutter }]}> 
      <View><Text style={styles.radiusTitle}>내 위치 기준</Text><Text style={styles.radiusDescription}>반경을 바꾸면 주변 매장을 다시 찾아요</Text></View>
      <Pressable accessibilityLabel={`검색 반경 ${formatRadius(radiusKm)}`} accessibilityHint="검색 반경 설정을 엽니다" accessibilityRole="button" onPress={() => setRadiusPickerOpen(true)} style={({ pressed }) => [styles.radiusPickerButton, pressed && styles.pressed]}><Ionicons name="options-outline" size={16} color={colors.ink900}/><Text style={styles.radiusPickerValue}>{formatRadius(radiusKm)}</Text><Ionicons name="chevron-down" size={14} color={colors.ink500}/></Pressable>
    </View>
    {featured.length ? <View style={styles.featuredSection}>
      <View style={[styles.sectionHeading, { paddingHorizontal: gutter }]}><View><Text style={styles.sectionTitle}>지금 할인 중</Text><Text style={styles.sectionDescription}>{formatRadius(radiusKm)} 안에서 바로 픽업할 수 있어요</Text></View><Text style={styles.sectionCount}>{featured.length}곳</Text></View>
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={[styles.featuredList, { paddingHorizontal: gutter }]}>{featured.map((store) => <FeaturedStore key={store.storeId} store={store} width={featureWidth}/>)}</ScrollView>
    </View> : null}
    <View style={styles.sectionDivider}/>
    <View style={[styles.listHeading, { paddingHorizontal: gutter }]}><Text style={styles.listTitle}>{query ? '검색 결과' : '거리순'}</Text><View style={styles.resultBadge}><Ionicons name="location" size={13} color={colors.ink700}/><Text style={styles.resultText}>{formatRadius(radiusKm)} · {filteredStores.length}곳</Text></View></View>
  </View>;
  ...
  return <SafeAreaView style={styles.safe} edges={['top']}>{topBar}<LinearGradient colors={[colors.canvas, colors.canvasWarm, colors.white]} end={{ x: 0.5, y: 1 }} pointerEvents="none" start={{ x: 0.5, y: 0 }} style={styles.topFade}/><RefreshStatus visible={refreshing}/><FlatList
    ...
    renderItem={({ item }) => <View style={{ paddingHorizontal: gutter }}><StoreCard compact store={item} onPress={() => router.push({ pathname: '/stores/[storeId]', params: { storeId: String(item.storeId), origin: '/stores' } })}/></View>}
    refreshControl={<AppRefreshControl refreshing={refreshing} onRefresh={onRefresh}/>}
    showsVerticalScrollIndicator={false}
    ListEmptyComponent={loading ? <LoadingState label="주변 마감 매장을 찾고 있어요"/> : <EmptyState title={query ? '검색 결과가 없어요' : '주변 상점을 찾지 못했어요'} description={query ? '검색어나 카테고리를 바꿔보세요.' : '검색 반경을 넓혀보세요.'}/>} 
...
const styles = StyleSheet.create({
  ...
  topBar: { alignSelf: 'center', paddingTop: 10, paddingBottom: 8, flexDirection: 'row', alignItems: 'center', gap: 5, backgroundColor: colors.canvas }, topFade: { height: 22 }, search: { minWidth: 0, height: 44, flex: 1, paddingLeft: 7, paddingRight: 12, flexDirection: 'row', alignItems: 'center', gap: 8, borderRadius: 22, backgroundColor: colors.white, ...shadow.float }, searchInput: { minWidth: 0, flex: 1, height: 44, paddingVertical: 0, color: colors.ink900, fontFamily: fonts.body, fontSize: 14, fontWeight: '600' }, headerActions: { flexDirection: 'row', gap: 5, backgroundColor: 'transparent' }, iconTouch: { width: 42, height: 42, alignItems: 'center', justifyContent: 'center', backgroundColor: 'transparent' }, iconSurface: { width: 38, height: 38, alignItems: 'center', justifyContent: 'center', borderRadius: 19, backgroundColor: colors.white, borderWidth: StyleSheet.hairlineWidth, borderColor: 'rgba(20,28,22,.08)' }, cartBadge: { position: 'absolute', right: 0, top: -3, minWidth: 16, height: 16, paddingHorizontal: 3, alignItems: 'center', justifyContent: 'center', borderRadius: 8, backgroundColor: colors.green500, borderWidth: 1.5, borderColor: colors.white }, cartBadgeText: { color: colors.ink900, fontFamily: fonts.body, fontSize: 10, fontWeight: '700' },
  ...
  radiusSection: { marginTop: 14, padding: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 12, borderRadius: radius.input, backgroundColor: colors.canvas }, radiusTitle: { color: colors.ink900, fontFamily: fonts.body, fontSize: 13, fontWeight: '800' }, radiusDescription: { marginTop: 2, color: colors.ink500, fontFamily: fonts.body, fontSize: 10 }, radiusPickerButton: { minWidth: 94, minHeight: 44, paddingHorizontal: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, borderRadius: radius.control, backgroundColor: colors.white, borderWidth: 1, borderColor: colors.lineStrong }, radiusPickerValue: { color: colors.ink900, fontFamily: fonts.body, fontSize: 13, fontWeight: '900' },
  featuredSection: { paddingTop: 21, paddingBottom: 20 }, sectionHeading: { marginBottom: 12, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end' }, sectionTitle: { color: colors.ink900, fontFamily: fonts.body, fontSize: 20, fontWeight: '900', letterSpacing: -0.6 }, sectionDescription: { marginTop: 4, color: colors.ink500, fontFamily: fonts.body, fontSize: 12 }, sectionCount: { color: colors.ink700, fontFamily: fonts.body, fontSize: 12, fontWeight: '700' }, featuredList: { gap: 10 }, featureCard: { overflow: 'hidden', borderRadius: radius.card, backgroundColor: colors.white, borderWidth: 1, borderColor: colors.line, ...shadow.card }, featureImage: { height: 118, backgroundColor: colors.canvas }, featureCopy: { padding: 11 }, featureName: { color: colors.ink900, fontFamily: fonts.body, fontSize: 15, fontWeight: '800' }, featureMeta: { marginTop: 4, color: colors.ink500, fontFamily: fonts.body, fontSize: 11 }, featurePrice: { marginTop: 8, color: colors.ink900, fontFamily: fonts.body, fontSize: 15, fontWeight: '900' },
  sectionDivider: { height: 10, backgroundColor: colors.canvas }, listHeading: { paddingTop: 18, paddingBottom: 11, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }, listTitle: { color: colors.ink900, fontFamily: fonts.body, fontSize: 17, fontWeight: '900' }, resultBadge: { flexDirection: 'row', alignItems: 'center', gap: 4 }, resultText: { color: colors.ink700, fontFamily: fonts.body, fontSize: 12, fontWeight: '700' }, pressed: { opacity: 0.72, transform: [{ scale: 0.99 }] },
