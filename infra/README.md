...
│   ├── compose.yaml                 # PostgreSQL·Elasticsearch·Exporter·Alloy
    ├── .env.example                 # 운영자가 설정할 환경변수 이름
    ├── elasticsearch/               # Elasticsearch JVM 설정
│   └── alloy/                       # Docker 컨테이너 로그 수집 설정
└── ec2-log/                         # Home Mini PC 관측 서버
    ├── compose.yaml                 # Loki·Prometheus·Grafana·SSH tunnel
    └── grafana/                     # 데이터 소스·대시보드 provisioning
```

## 서버별 책임

| 구분 | Main EC2 | Data EC2 | EC2 Log (Home) |
| --- | --- | --- | --- |
| 실행 방식 | Kubernetes | Docker Compose | Docker Compose |
| 애플리케이션 | Gateway, Config, Member, Core, Payment, AI | 없음 | 없음 |
| 데이터 인프라 | Redis, Kafka | PostgreSQL, Elasticsearch | 없음 |
| 관측 | Alloy, kube-state-metrics, Node Exporter | Exporter, Alloy, cAdvisor | Loki, Prometheus, Grafana |

## 설정 관리 원칙

- `infra/ec2-main`, `infra/ec2-data`, `infra/ec2-log`가 배포 설정의 원본입니다. 서버에서 직접 수정한 내용은 저장소에도 반드시 반영합니다.
- Home Grafana 대시보드를 UI나 서버에서 수정한 경우 `infra/ec2-log/grafana/dashboards/*.json`도 같은 작업 안에서 갱신합니다. 상세 절차는 `ec2-log/README.md`를 따릅니다.
...
Home Mini PC의 DNS·방화벽·데이터 디렉터리와 Main EC2 제한 SSH 터널을 준비한 뒤 [`ec2-log/README.md`](ec2-log/README.md)의 자격증명 생성, 데이터 이전, 절체 순서를 따른다.
