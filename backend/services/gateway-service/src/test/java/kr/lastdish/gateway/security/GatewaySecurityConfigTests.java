...
class GatewaySecurityConfigTests {
  ...
  @Test
  void aiNearbyStoresRouteAllowsGetRequestsWithoutAuthentication() {
    webTestClient.get().uri("/api/v1/ai/stores/nearby").exchange().expectStatus().isOk();
  }
  ...
  static class TestRoutes {
    ...
    RouterFunction<ServerResponse> securityTestRoutes() {
      return route(POST("/api/v1/auth/login"), request -> ok().build())
          ...
          .andRoute(GET("/api/v1/ai/stores/nearby"), request -> ok().build())
