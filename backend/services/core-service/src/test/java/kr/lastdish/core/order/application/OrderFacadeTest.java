...
class OrderFacadeTest {
    ...
    void payAndCreateOrder_success() {
        ...
        when(orderService.completePayment(10L))
                .thenReturn(expectedResponse);

        // when
        OrderResponse response =
                orderFacade.payAndCreateOrder(memberId, request);

        // then
        assertThat(response).isSameAs(expectedResponse);

        InOrder inOrder =
                inOrder(orderService, dishService, depositService);

        inOrder.verify(orderService)
                .createOrder(memberId, request);

        inOrder.verify(dishService)
                .decreaseStock(100L, 2L);

        inOrder.verify(depositService)
                .use(
                        memberId,
                        10L,
