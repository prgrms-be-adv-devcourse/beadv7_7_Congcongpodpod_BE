...
@Table(
    ...
    uniqueConstraints = {
      @UniqueConstraint(
          ...
          columnNames = {"store_id", "settlement_month"})
    },
    indexes = {
      @Index(name = "idx_settlement_store_created_at", columnList = "store_id, created_at"),
...
public class Settlement {
  ...
  private void validate(
      ...
      long settlementAmount) {
    if (storeId == null) {
      throw new BusinessException(CommonErrorCode.INVALID_INPUT, "매장 ID는 필수입니다.");
    }

    if (settlementMonth == null) {
      throw new BusinessException(CommonErrorCode.INVALID_INPUT, "정산 대상 월은 필수입니다.");
    }

    if (periodStart == null || periodEnd == null) {
      throw new BusinessException(CommonErrorCode.INVALID_INPUT, "정산 기간은 필수입니다.");
    }

    if (!periodStart.isBefore(periodEnd)) {
      throw new BusinessException(CommonErrorCode.INVALID_INPUT, "정산 시작 시각은 종료 시각보다 이전이어야 합니다.");
    }

    if (totalOrderCount < 0 || grossAmount < 0 || feeAmount < 0 || settlementAmount < 0) {
      throw new BusinessException(CommonErrorCode.INVALID_INPUT, "정산 건수와 금액은 음수일 수 없습니다.");
    }
    ...
    if (settlementAmount != grossAmount - feeAmount) {
      throw new BusinessException(CommonErrorCode.INVALID_INPUT, "정산 금액이 총 판매 금액과 수수료에 일치하지 않습니다.");
    }
  }
