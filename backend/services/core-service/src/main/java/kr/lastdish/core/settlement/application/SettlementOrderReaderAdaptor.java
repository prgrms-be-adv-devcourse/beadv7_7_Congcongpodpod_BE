...
public class SettlementOrderReaderAdaptor implements SettlementOrderReader {
  ...
  @Override
  public List<SettlementOrderData> readSettlementOrders(
      Long storeId, LocalDateTime periodStart, LocalDateTime periodEnd) {
    return orderFacade.findSettlementOrders(storeId, periodStart, periodEnd).stream()
        .map(this::toSettlementOrderData)
        .toList();
  }
