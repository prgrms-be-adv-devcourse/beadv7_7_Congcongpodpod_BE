...
public class SettlementTransactionalManager {
    ...
    public void complete(Long settlementId, List<OrderSettlementAmount> calculatedOrders){
        ...
        long totalSalesAmount = calculatedOrders.stream().mapToLong(OrderSettlementAmount::salesAmount).sum();
        long totalFeeAmount = calculatedOrders.stream().mapToLong(OrderSettlementAmount::feeAmount).sum();
        long totalSettlementAmount = calculatedOrders.stream().mapToLong(OrderSettlementAmount::settlementAmount).sum();


        settlement.updateCalculation(calculatedOrders.size(), totalSalesAmount, totalFeeAmount, totalSettlementAmount);

        List<SettlementDetail> settlementDetailList = calculatedOrders.stream()
                .map(calculated -> new SettlementDetail(
                        settlement.getId(),
    ...
    private Settlement findSettlement(Long settlementId){
        return settlementRepository.findById(settlementId)
                .orElseThrow(() -> new BusinessException(CommonErrorCode.ENTITY_NOT_FOUND, "정산 정보를 찾을 수 없습니다."));
    }
