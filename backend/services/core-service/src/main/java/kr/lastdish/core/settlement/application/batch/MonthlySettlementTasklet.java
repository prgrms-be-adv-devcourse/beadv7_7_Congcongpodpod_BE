...
public class MonthlySettlementTasklet implements Tasklet {
    ...
    public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) {
        String monthValue = contribution
                .getStepExecution()
                .getJobParameters()
                .getString("settlementMonth");

        if (monthValue == null || monthValue.isBlank()){
            throw new IllegalArgumentException("settlementMonth는 필수입니다.");
        }

        YearMonth settlementMonth = YearMonth.parse(monthValue);

        List<Long> storeIds = settlementStoreReader.readSettlementTagertStoreIds();
        ...
        for (Long storeId : storeIds) {
            try {
                SettlementCreateResult result = settlementService.createMonthlySettlement(storeId, settlementMonth);
                ...
                failedCount++;
                log.error("월 정산 생성 실패. storeId={}, settlementMonth={}", storeId, settlementMonth, exception);
            }
        }
