...
public class SettlementEventAccumulator {
  ...
  public void accumulate(
      Long orderId, Long storeId, BigDecimal totalPrice, LocalDateTime orderCompletedAt) {
    ...
    long settlementAmount =
        settlementCalculator.calculateSettlementAmount(totalPrice.longValueExact(), feeAmount);
  ...
  private Settlement findOrCreateAccumulatingSettlement(
      Long storeId, YearMonth settlementMonth, SettlementPeriod period) {
    Optional<Settlement> existing =
        settlementRepository.findByStoreIdAndSettlementMonth(storeId, settlementMonth);
    ...
    return settlementRepository
        .findByStoreIdAndSettlementMonth(storeId, settlementMonth)
        .orElseThrow(
            () ->
                new BusinessException(CommonErrorCode.ENTITY_NOT_FOUND, "이벤트 정산 정보를 생성하지 못했습니다."));
  }
  ...
  private void validateAccumulating(Settlement settlement) {
    if (settlement.getSettlementStatus() != SettlementStatus.ACCUMULATING) {
      throw new BusinessException(
          CommonErrorCode.INVALID_STATE, "적립 중인 정산에만 주문 정산 내역을 추가할 수 있습니다.");
    }
  }
}
