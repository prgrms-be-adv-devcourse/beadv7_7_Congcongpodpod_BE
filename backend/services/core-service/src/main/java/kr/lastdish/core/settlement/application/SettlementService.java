...
public class SettlementService {
    ...
    public SettlementCreateResult createMonthlySettlement(
            Long storeId,
    ...
    private List<SettlementOrderData> excludeSettledOrders(List<SettlementOrderData> orders) {
        ...
        return orders.stream()
                .filter(order -> !settledOrderIds.contains(order.orderId()))
                .toList();
    }
    ...
    private List<OrderSettlementAmount> calculateOrders(List<SettlementOrderData> orders) {
        ...
        return orders.stream()
                .map(order -> calculateOrder(order, feeRate))
                .toList();
    }
    ...
    private OrderSettlementAmount calculateOrder(SettlementOrderData order, BigDecimal feeRate) {
        ...
        long settlementAmount = settlementCalculator.calculateSettlementAmount(order.salesAmount(), feeAmount);
    ...
    private Settlement createSettlement(
        ...
        // 정산 전체 판매액 합산
        long grossAmount = calculatedOrders.stream()
                .mapToLong(OrderSettlementAmount::salesAmount)
                .sum();

        // 정산 전체 수수료 합산
        long feeAmount = calculatedOrders.stream()
                .mapToLong(OrderSettlementAmount::feeAmount)
                .sum();

        // 정산 전체 정산액 합산
        long settlementAmount = calculatedOrders.stream()
                .mapToLong(OrderSettlementAmount::settlementAmount)
                .sum();
