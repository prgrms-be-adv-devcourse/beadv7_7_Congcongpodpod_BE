...
public record SettlementOrderData(
    Long orderId, Long storeId, Long salesAmount, LocalDateTime orderCompletedAt) {
  public SettlementOrderData {
    ...
    if (salesAmount < 0) {
      throw new IllegalArgumentException("판매 금액은 음수일 수 없습니다.");
    }

    if (orderCompletedAt == null) {
      throw new IllegalArgumentException("주문 완료 시각은 필수입니다.");
    }
  }
}
